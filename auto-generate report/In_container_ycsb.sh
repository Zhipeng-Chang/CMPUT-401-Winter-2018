...
for i in "${arr[@]}"
do
./ycsb-0.12.0/bin/ycsb load cassandra-cql -p hosts=${IP_address} -P workloads/workload${i} -s > ../workload_load_output${i}.txt
./ycsb-0.12.0/bin/ycsb run  cassandra-cql -p hosts=${IP_address} -P workloads/workload${i} -s > ../workload_run_output${i}.txt
...
echo "report generated finished"
exit 1
